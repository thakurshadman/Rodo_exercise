...
List queryResults(List<dynamic> cars, String searchFieldInput) {
  ...
  if (searchMakeModel.length == 1){
    for (final car in cars) {
      if (car["make"].contains(RegExp('^'+ searchMakeModel[0], caseSensitive: false)) | car["model"].contains(RegExp('^'+searchMakeModel[0], caseSensitive: false))) {
        results.add(car);
      }
    }
  }
    ...
    for (final car in cars){
      if (car["make"].contains(RegExp('^'+searchMakeModel[0], caseSensitive: false)) & car["model"].contains(RegExp('^'+searchMakeModel[1], caseSensitive: false))){
        results.add(car);
      }
    }
  }
